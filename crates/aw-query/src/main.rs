...
fn fmt_unix_ns(ns: i64) -> String {
    // Seconds precision is plenty for a capture tool; render as unix seconds
    // plus a rough age so the reader doesn't have to do epoch math.
    let secs = ns / 1_000_000_000;
    let age_secs = (now_unix_ns() - ns) / 1_000_000_000;
    if age_secs >= 0 {
        format!("unix {secs} ({} ago)", fmt_duration_secs(age_secs as u64))
    } else {
        format!("unix {secs}")
    }
}

fn fmt_duration_secs(s: u64) -> String {
    if s >= 86_400 {
            ...
            if args.pretty {
                ...
                for s in &segs {
                    println!(
                        ...
                        s.app_name,
                        s.process_pid,
                        fmt_duration_secs(s.duration_secs),
